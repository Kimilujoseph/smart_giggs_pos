import { Expense_category, ExpensesPaymentMethod } from "@prisma/client";
import Joi from "joi";
...
const expenseInput = (data) => {
  const schema = Joi.object({
    ...
    paymentMethod: Joi.string()
      .valid(...Object.values(ExpensesPaymentMethod))
      .optional(),
    vendorName: Joi.string().trim().max(255).optional(),
    vendorContact: Joi.string().trim().max(255).optional(),
...
const analyticsQuery = (data) => {
  const schema = Joi.object({
    ...
    groupBy: Joi.string()
      .valid("category", "subcategory", "paymentMethod", "shop")
      .default("category"),
