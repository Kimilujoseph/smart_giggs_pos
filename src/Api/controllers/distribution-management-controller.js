  ...
  try {
    ...
    if (!bulkDistribution || bulkDistribution.length === 0) {
      throw new ValidationError(
        "Bulk distribution data is required and cannot be empty."
      );
    }

    if (category === "mobiles") {
      const successfulDistributions =
        await distributionManager.createBulkMobileDistribution({
          bulkDistribution,
      ...
      const successfulDistributions =
        await distributionManager.createBulkAccessoryDistribution({
          bulkDistribution,
          mainShop: shopDetails.mainShop,
          distributedShop: shopDetails.distributedShop,
          userId: parseInt(user.id, 10),
