...
class ReturnRepository {
  async createReturn(returnData) {
    ...
    return prisma.$transaction(async (tx) => {
      const saleModel =
        saleType === "mobile" ? tx.mobilesales : tx.accessorysales;
      const itemModel =
        saleType === "mobile" ? tx.mobileItems : tx.accessoryItems;
      ...
      if (!originalSale) {
        throw new APIError(
          ...
          `Original ${saleType} sale with ID ${saleId} not found.`
        );
      }
      ...
      if (quantity > originalSale.quantity) {
        throw new APIError(
          "Bad Request",
          STATUS_CODE.BAD_REQUEST,
          `Cannot return ${quantity} items. Only ${originalSale.quantity} available to return.`
        );
      }
      ...
      const commissionPerUnit =
        Number(originalSale.commission) / originalQuantity;
      const financeAmountPerUnit =
        Number(originalSale.financeAmount) / originalQuantity;
      ...
      if (restock) {
        if (saleType === "mobile") {
          const soldItem = await tx.mobileItems.findFirst({
            where: {
              mobileID: originalSale.productID,
          ...
          await tx.accessoryItems.updateMany({
            where: {
              accessoryID: originalSale.productID,
              shopID: originalSale.shopID,
            },
      ...
      const productDetails = await productModel.findUnique({
        where: { id: originalSale.productID },
      });
